#!/usr/bin/env bash
# Dev helper: create a Stage B session that lands on B_ASYNC_VIDEO (pre-seeds B_CODING through B_WORK_SAMPLE as done).
set -euo pipefail
...
FROM s
CROSS JOIN (VALUES
  ('B_CODING'::app.stage_key),
  ('B_DEBUG'::app.stage_key),
  ('B_WORK_SAMPLE'::app.stage_key)
) AS k(stage_key);
