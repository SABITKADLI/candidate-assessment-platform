#!/usr/bin/env bash
# Dev helper: create a session that lands on A_SJT (pre-seeds A_RESUME through A_INTEGRITY as done).
set -euo pipefail
...
CROSS JOIN (VALUES
  ...
  ('A_ID_LIVENESS'::app.stage_key),
  ('A_GMA'::app.stage_key),
  ('A_BIG5'::app.stage_key),
  ('A_MBTI'::app.stage_key),
  ('A_RORSCHACH'::app.stage_key),
  ('A_INTEGRITY'::app.stage_key)
) AS k(stage_key);
