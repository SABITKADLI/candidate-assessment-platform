...
export function IntegrityPlayer() {
  const [answers, setAnswers] = useState<Record<string, LikertVal>>({});
  const [page, setPage] = useState(0);
  const [phase, setPhase] = useState<Phase>('questions');
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  const pageItems = INTEGRITY_ITEMS.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);
  const pageAnswered = pageItems.every((item) => answers[item.id] != null);
  const totalAnswered = Object.keys(answers).length;
  const progress = Math.round((totalAnswered / INTEGRITY_ITEMS.length) * 100);
  ...
  async function finish() {
    ...
    setPhase('done');
    const tokenPath = window.location.pathname.replace(/\/a_integrity$/, '');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 12, color: 'var(--cap-fg-2)' }}>
          ...
          <span>{totalAnswered} / {INTEGRITY_ITEMS.length} answered</span>
        </div>
      ...
      {/* Likert header */}
      <div style={{
        display: 'grid', gridTemplateColumns: '1fr repeat(5, 72px)',
        gap: 4, fontSize: 11, color: 'var(--cap-fg-2)', textAlign: 'center',
        paddingBottom: 8, borderBottom: '1px solid var(--cap-border)',
      }}>
        <span />
        {LIKERT.map((l) => <span key={l.value} style={{ lineHeight: 1.2 }}>{l.label}</span>)}
      </div>
      ...
      {pageItems.map((item, idx) => {
        ...
        return (
          <div key={item.id} style={{
            ...
            <span style={{ fontSize: 14, color: 'var(--cap-fg-1)', paddingRight: 12 }}>{item.text}</span>
            {LIKERT.map((l) => {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 8 }}>
        ...
        {isLastPage ? (
          <Button variant="primary" disabled={!pageAnswered} onClick={finish}>Submit</Button>
        ) : (
